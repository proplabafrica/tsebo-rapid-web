...
export default {
  ...
  methods: {
    ...
    getChartMaterial() {
      ...
      //         name: materialList?.material?.name,
      ...
      const rearrangedMaterial =
        this.rearrangeMaterialByMonthYear(restructuredData);
      ...
      for (const key in materialArray) {
        ...
        if (month === targetMonth && year === targetYear) {
          ...
          for (const itemName in items) {
            this.materialNames.push(itemName);
            this.materialCounts.push(items[itemName]);
          }
        }
      }
    ...
    //     },
    //     dataLabels: { enabled: false },
    //     xaxis: { categories: this.materialNames },
    //     colors: ["#9a0101"]
    //   };
    ...
    //   this.chart = renderChartOptions;

    //   // Render the new chart
    //   this.chart.render();

    //   "object", this.materialCounts, this.materialNames);
    // },
    ...
    formatDate(dateTime) {
      ...
      ("formatting the date for the fixed date ", month, parseInt(date.split("-")[1]) - 1);
    ...
    async getRenderMaterials() {
      ...
      try {
        ...
        (this.plottedMaterials, "items... plotted mat");
