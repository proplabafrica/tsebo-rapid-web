  ...
  <div>
    <form id="create-maintenance-plan-form" class="bg-white pd-20">
      <div class="row">
        <div class="col-md-4 form-group">
          ...
          <input type="text" class="form-control" name="name" placeholder="Short Description"
            v-model="maintenancePlan.name" required />
        ...
        <div class="col-md-8 form-group">
          ...
          <textarea class="form-control" name="description" placeholder="Description"
            v-model="maintenancePlan.description" required></textarea>
        ...
        <div class="col-md-6 col-lg-4 form-group">
          ...
          <div class="input-group">
            <!-- <input
              ...
              required
            /> -->
            <select name="frequency_measurement" v-model="maintenancePlan.frequency_measurement" class="form-control"
              required>
          ...
          <small class="tx-bold tx-danger" v-if="validationErrors.frequency"
            v-text="validationErrors.frequency"></small>
        ...
        <div class="col-md-6 col-lg-4 form-group">
          ...
          <div class="input-group">
            <input type="number" name="time_to_reminder" placeholder="Time to reminder" class="form-control"
              v-model="maintenancePlan.time_to_reminder" required />
            <select name="reminder_measurement" v-model="maintenancePlan.reminder_measurement" class="form-control"
              required>
              <option :value="null">Select Unit</option>
              <option v-for="measurement in ['weeks']" :key="measurement" :value="measurement" v-text="measurement">
              </option>
            </select>
          </div>
          <small class="tx-bold tx-danger" v-if="validationErrors.frequency"
            v-text="validationErrors.frequency"></small>
          <small class="tx-bold tx-danger float-right" v-if="validationErrors.reminder_measurement_id"
            v-text="validationErrors.reminder_measurement_id"></small>
        </div>
        <div class="col-md-6 col-lg-4 form-group">
          <label class="form-control-label">
            Select Trade
          ...
          <v-select class="form-control" label="name" placeholder="Select Trade" selectOnTab :options="trades"
            v-model="maintenancePlan.trade"></v-select>
        </div>
        <div class="col-md-6 col-lg-4 form-group">
          <label class="form-control-label">
            Start Date
      ...
      <div class="form-layout-footer tx-center">
        <v-button type="button" class="btn btn-primary pd-x-25" :disabled="disabled" @click="submitMaintenancePlan()">
          SUBMIT</v-button>
      </div>
    </form>
  </div>
...
export default {
  ...
  data: () => getInitialData(),
  head: () => ({
    title: "Create Unit Scheule · Tsebo-Rapid"
  }),
  meta: {
  ...
  methods: {
    ...
    async submitJobSchedule() {
      ...
      try {
        ...
        this.$emit("update");
        this.$router.push(
          `/location/units/details/schedules?id=${this.unit.id}`
        );
      } catch (error) {
        this.disabled = false;
    ...
    async submitMaintenancePlan() {
      if (!this.validateForm("create-maintenance-plan-form")) return false;
      this.disabled = true;

      this.maintenancePlan.frequency = 1;

      const trade_ids = [this.maintenancePlan.trade.id];
      const unit_ids = [this.unit.id];
